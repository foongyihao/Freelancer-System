...
import { refreshList, currentPage, pageSize } from '/feature/freelancer/state_freelancer.js';
...
async function loadSelectOptions(selectId, url, opts={}){
  ...
  select.innerHTML='';
  // Add a disabled placeholder row like "Select... [hold CMD for multiple]"
  ...
  items.forEach(x=>{
    ...
    opt.value = x.name; opt.textContent = x.name; select.appendChild(opt);
  });
}
...
document.addEventListener('DOMContentLoaded', function() {
  const withState = (fn) => import('/feature/freelancer/state_freelancer.js').then(m=>fn(m));
  ...
  // Picker: hobbies multi-select -> merge into #hobbies
  ...
  // Search filters: use dropdowns too (no auto-search on change)
  ...
  // Intentionally no change listeners; search runs only when Search button is clicked
  // Initial load
  refreshList();
});
